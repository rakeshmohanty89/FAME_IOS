...
public class TC_32 extends Lib_SeeTest{
	public void Test_TC_32(Lib_Global ogbl) throws Exception {
		try {
			...
			if(oAppLib.Functionality_Select("Book Flight")){			
				...
				if(oGbl.Failureflag == 0){
					...
					if(!PNRno.equalsIgnoreCase(""))
					{
						...
						//String Parameters[] = {"VERIFY",PNRno,TD("Passenger1_Fname"),TD("Passenger1_Lname"),"N/A","N/A",TD("Passenger1_Mobile"),TD("Passenger1_Email").toUpperCase().replace("@", "//"),TD("street").toUpperCase(),TD("city").toUpperCase(),TD("postalcode").toUpperCase(),TD("Flight_no"),TD("Departure_dt").substring(0,6).toUpperCase().replace(" ", "").replace("-", ""),TD("Origin_airport"),"SeatNo","N/A","N/A"};
